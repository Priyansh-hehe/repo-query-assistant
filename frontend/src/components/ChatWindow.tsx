...
export default function ChatWindow({
  ...
  return (
    <div className="flex flex-col h-[650px] bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm dark:shadow-xl overflow-hidden backdrop-blur-sm transition-colors">
      {/* Chat Messages Feed */}
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-4">
            <div className="h-12 w-12 rounded-2xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center text-cyan-600 dark:text-cyan-400">
              <svg
                className="w-6 h-6"
                fill="none"
                stroke="currentColor"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-md pt-2">
              {STARTER_PROMPTS.map((prompt, idx) => (
                <button
                  ...
                  className="text-left text-xs p-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300 hover:border-cyan-500/50 hover:bg-zinc-100 dark:hover:bg-zinc-900/80 transition-all disabled:opacity-40"
                >
                  &ldquo;{prompt}&rdquo;
                </button>
              ))}
          ...
          messages.map((msg) => (
            ...
            >
              {/* Message Header */}
              <div className="flex items-center gap-2 mb-1 text-[11px] text-zinc-400 dark:text-zinc-500 font-mono">
                <span>{msg.sender === "user" ? "You" : "Code Query AI"}</span>
                <span>&bull;</span>
        ...
        {isLoading && (
          <div className="flex flex-col items-start space-y-2">
            ...
            <div className="p-4 rounded-2xl bg-zinc-100 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 text-xs flex items-center gap-3">
      ...
      <form
        ...
        className="p-3 sm:p-4 bg-zinc-50 dark:bg-zinc-950 border-t border-zinc-200 dark:border-zinc-800/80 flex items-center gap-3"
      >
        <textarea
          rows={1}
          value={inputQuestion}
          ...
          disabled={isLoading || !selectedRepo}
          className="flex-1 px-4 py-2.5 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-500 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 resize-none transition-all disabled:opacity-50"
        />

        <button
          type="submit"
          disabled={isLoading || !inputQuestion.trim() || !selectedRepo}
          className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white text-sm font-medium shadow-lg shadow-cyan-500/20 transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2 flex-shrink-0"
        >
          <span>Ask</span>
          <svg
            className="w-4 h-4"
